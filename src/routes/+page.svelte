...
<div class="root" class:mounted>
	...
	<section class="section-light platforms-section">
		<div class="container">
			...
			<div class="platforms-carousel" aria-label="Supported platforms carousel">
				<div class="platforms-fade platforms-fade-left" aria-hidden="true"></div>
				<div class="platforms-fade platforms-fade-right" aria-hidden="true"></div>

				<div class="platforms-track">
					{#each [...platforms, ...platforms] as p, i}
						<div class="platform-pill" style="--accent:{p.color}; --delay:{i * 0.03}s">
							<div class="platform-pill-icon" aria-hidden="true">
								{#if p.icon === 'instagram'}
									<svg viewBox="0 0 24 24" width="24" height="24" fill="none">
										<rect width="24" height="24" rx="6" fill="url(#ig-g2)"/>
										<defs><linearGradient id="ig-g2" x1="0" y1="24" x2="24" y2="0">
								...
								{:else if p.icon === 'facebook'}
									<svg viewBox="0 0 24 24" width="24" height="24" style="color:{p.color}" fill="currentColor">
								...
								{:else if p.icon === 'x'}
									<svg viewBox="0 0 24 24" width="24" height="24" fill="#0a0a0a">
								...
								{:else if p.icon === 'bluesky'}
									<svg viewBox="0 0 24 24" width="24" height="24" style="color:{p.color}" fill="currentColor">
										<path d="M5.202 2.857C7.954 4.922 10.913 9.11 12 11.358c1.087-2.247 4.046-6.436 6.798-8.501C20.783 1.366 24 .213 24 3.883c0 .732-.42 6.156-.667 7.037-.856 3.061-3.978 3.842-6.755 3.37 4.854.826 6.089 3.562 3.422 6.299-5.065 5.196-7.28-1.304-7.847-2.97-.104-.305-.152-.448-.153-.327 0-.121-.05.022-.153.327-.568 1.666-2.782 8.166-7.847 2.97-2.667-2.737-1.432-5.473 3.422-6.3-2.777.473-5.899-.308-6.755-3.369C.42 10.04 0 4.615 0 3.883c0-3.67 3.217-2.517 5.202-1.026"/>
									</svg>
								{:else}
									<div class="platform-fallback" style="--accent:{p.color}">{p.abbr ?? p.name.slice(0,1)}</div>
								{/if}
							</div>
							<span class="platform-pill-name">{p.name}</span>
						</div>
					{/each}
				</div>
			</div>
...
.platforms-carousel {
	position: relative;
	border-radius: 22px;
	background: rgba(10,5,5,0.02);
	border: 1px solid rgba(10,5,5,0.10);
	overflow: hidden;
}
.platforms-track {
	display: flex;
	...
	padding: 18px 18px;
	width: max-content;
	animation: platforms-marquee 26s linear infinite;
	will-change: transform;
}
.platforms-carousel:hover .platforms-track { animation-play-state: paused; }
...
.platform-pill {
	...
	border: 1px solid rgba(10,5,5,0.10);
	box-shadow: 0 8px 20px rgba(10,5,5,0.06);
	white-space: nowrap;
}
.platform-pill-icon {
	width: 34px;
	...
	align-items: center;
	justify-content: center;
	background: color-mix(in srgb, var(--accent, #111) 10%, white);
	border: 1px solid color-mix(in srgb, var(--accent, #111) 22%, rgba(10,5,5,0.12));
}
.platform-pill-name {
	font-size: 12px;
	font-weight: 600;
	color: rgba(10,5,5,0.70);
}

.platform-pill svg { display: block; }
.platform-fallback {
	width: 34px;
	height: 34px;
	border-radius: 12px;
	...
	background: color-mix(in srgb, var(--accent, #111) 16%, white);
	border: 1px solid color-mix(in srgb, var(--accent, #111) 30%, rgba(10,5,5,0.12));
}

/* Platforms in light section */
.section-light .platforms-carousel { background: rgba(10,5,5,0.02); border-color: rgba(10,5,5,0.10); }
...
@media (max-width: 900px) {
	...
	.platforms-fade { width: 42px; }
	.platforms-track { padding: 14px 14px; animation-duration: 22s; }
	.platform-pill { padding: 10px 14px; }
	.platform-pill-name { font-size: 11px; }
...
@media (max-width: 600px) {
	...
	.platforms-fade { width: 34px; }
	.platforms-track { padding: 12px 12px; animation-duration: 18s; }
	.platform-pill-icon { width: 32px; height: 32px; border-radius: 11px; }
